...
public class BotI {
    ...
    // NOTE: Changed method from private -> public for the sake of visibility in
    // BotITest.java
    ...
    // Takes the current board state (8x8), attempts to collect all available moves,
    // then either:
    ...
    // - Pick standard move if no capture moves available. If multiple are
    // available, pick at random
    // - If no moves available, return null.
    public Move generateMove(char[][] currentBoard) {
        Random random = new Random();

        // Pick capture move if available. If multiple are available, pick at random
        ArrayList<Move> availableCaptureMoves = getAvailableCaptureMoves(currentBoard);
        ...
        // Pick standard move if no capture moves available. If multiple are available,
        // pick at random
 ...
 // this method runs when the user makes a move.
    // it tries to generate a bot move and make the bot play next.
    public void onUserMove(Pieces currentBoard, Move userMove) {
        // call the method that figures out what move the bot should make
        Move botMove = generateMove(currentBoard);

        // if no moves are possible for the bot, then the game is most likely over
        if (botMove == null) {
            // TODO: Add code to end the game if needed
            return;
        }

        // If the bot has a valid move, make the move using gameManager
        // (this sends the move to the game logic)
        // TODO: actually call the method to make the bot move
        // for example: gameManager.processMove(botMove);
    }
}